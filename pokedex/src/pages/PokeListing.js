...
const PokeListing = () => {
    ...
    console.log(Object.values(mock_data.abilities));

    
    return (
        
        <Wrapper>
            <PokeName>{mock_data.name}</PokeName>
            
            <ImageInfoContainer>
                <SpriteContainer>
                    <Sprite src={mock_data.sprites.other.dream_world.front_default} alt={mock_data.name}/>
                </SpriteContainer>

                <GeneralInfoContainer>
                    <GeneralInfoName>National №: </GeneralInfoName>
                    <NationalNumber>{mock_data.id}</NationalNumber>

                    <GeneralInfoName>Type: </GeneralInfoName>
                    <TypeContainer>
                        {mock_data.types.map( (type) => <Type>{type.type.name}</Type>)}
                    </TypeContainer>

                    <GeneralInfoName>Height: </GeneralInfoName>
                    <HeightWeight>{mock_data.height/10} m</HeightWeight>

                    <GeneralInfoName>Weight: </GeneralInfoName>
                    <HeightWeight>{mock_data.weight/10} kg</HeightWeight>

                    <GeneralInfoName>Abilities: </GeneralInfoName>
                    <AbilitiesContainer>
                        {/* object . values ability and show them, if it works do the same with types */}
                        {mock_data.abilities.map( ({ability}) => <Ability>{ability.name}</Ability>)}
                    </AbilitiesContainer>
